...
Two flavors are generated at build time by `scripts/build-og.mjs`:

1. **Default site card** — rendered from `public/og-default.svg` →
   `public/og-default.png`. Used on the home page and any non-detail page.
2. **Per-entry cards** — one PNG per case study and per article, written to
   `public/og/<collection>/<slug>.png`. The case-study and article detail
   pages reference these via `og:image`, so a shared link to a case study
   shows the case study's title and tags in the preview card, not the
   generic site card.

The script uses [`sharp`](https://sharp.pixelplumbing.com/) (a transitive
Astro dependency) and [`gray-matter`](https://github.com/jonschlinkert/gray-matter)
to read MDX frontmatter. It runs automatically as `prebuild`, so `pnpm build`
always emits fresh PNGs.

`public/og/` is gitignored — per-entry PNGs are pure build output.

To regenerate manually after editing the SVG template or content frontmatter:
...
Each page emits one or more `<script type="application/ld+json">` blocks
(schema.org JSON-LD):

- The home page emits `Person` + `WebSite`.
- Case study detail pages emit `TechArticle`.
- Article detail pages emit `Article`.

Helpers live in `src/lib/meta.ts` (`personSchema`, `websiteSchema`,
`articleSchema`). To verify what search engines see, paste a deployed URL
into Google's [Rich Results Test](https://search.google.com/test/rich-results).

## RSS feed

Combined feed at `/rss.xml` covering both case studies and articles,
newest-first. Generated by `src/pages/rss.xml.ts` via `@astrojs/rss`.

Linked from:

- The footer (small "RSS" link).
- A `<link rel="alternate" type="application/rss+xml">` in `<head>` so feed
